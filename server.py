...
HOST = 'localhost' # 서버의 ip를 열음. (이 서버의 ip로 클라이언트가 접속을 해야 한다), 그전에 ping을 먼저 확인하도록.
...
class MyTcpHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):  # 클라이언트가 접속시 클라이언트 주소 출력
        ...
        try:
            new_user = self.registerUsername()
            msg = self.request.recv(1024)
            while msg:
                print(msg.decode())
                if self.userman.messageHandler(new_user, msg.decode()) == -1:
                    self.request.close()
                    break
                msg = self.request.recv(1024)
